...
- **→ Tier S fully swept: S1 ✅ (null), S2 ✅ (already-captured), S3 ✅ (mtp-alone optimal). Tier A: A1 ✅
  ...
  **Tier A: A1/A3/A4 swept; A2 = strong win; A5 (trained EAGLE/DSpark draft) CLOSED NEGATIVE 2026-08-06
  (built+measured: crashes on VLMs, −81.6% on dense text vs the free native MTP — DSpark dead for our fleet).**
  Next: Track H (harness product), Track M (multimodal), or the A2 follow-on (our-own concise 35B-MoE via
  trace-distillation).**
...
### A5. Trained EAGLE-family draft (DFlash / DSpark / Eagle3) as an alternative to the native MTP head — §35 (sibling of S3/A1) — **✅ CLOSED NEGATIVE 2026-08-06: built + measured on all 3 fronts; DSpark is DEAD for our fleet — native MTP wins everywhere. Full record `runs/a5-dspark/RESULTS.md`.**
- **⚑⚑⚑ CLOSER (fable-tc dense, the last non-blocked test): DSpark LOADS on arch qwen35 but is −81.6%.** nospec 43.2 → native-MTP 55.0 t/s (+27%, accept 0.444) → **DSpark 7.9 t/s (accept 0.212), −81.6%** (satgeze/Qwen3.6-27B-DSpark, arch dflash upstream fmt). Two compounding killers: (1) accept COLLAPSE 0.212 vs MTP 0.444 — the draft is base-Qwen3.6-27B-trained, fable-tc is a MERGE → shifted hidden-states → mispredicts (the exact mismatch the native MTP tolerates because it rode through the merge); (2) draft cost DOMINATES — a 3.73GB **bf16** drafter with n_extract=5 (5 target-layer reads/step) on a **bandwidth-bound** 3090 crushes throughput vs the ~free in-model nextn head. **FINAL: EAGLE-family is target-feature-conditioned → crashes on VLM graphs (qwen3vl) AND loses badly to the free native MTP on text (heavy external draft on batch-1 bandwidth-bound HW). Native MTP/ngram are the right tools; DSpark buys nothing on this fleet.** Deploy stays `lifecycle` + native MTP; cherry-pick banked (inert, would only serve a future *vanilla* Qwen3 text model).
- **⚑⚑ STEP-2 HARD RESULT 2026-08-06 (built + measured, not reasoned): EAGLE-family drafts are INCOMPATIBLE with VLM targets in llama.cpp.** Cherry-picked #25173 onto probe branch `dspark-probe` (commit `4c9d7675a`, CLEAN — DFlash refactor #25110 already in-tree; binary now lists `draft-dspark`). Two VLM A/Bs on the DSpark binary:
  - **gemma-4-12b-vision:** incumbent MTP (QAT assistant) measured STRONG on reasoning text — nospec 83.0 → mtp 228.4 t/s = **+175%, accept 0.939**. DSpark arm: the public GGUF (`ankk98/dspark-gemma4-12b-block7`) is **format-incompatible** — made with the `ft-dspark` FORK (standalone arch `dspark`, keys `dspark.*`, tensors `markov.w1.weight`), but upstream #25173 folded DSpark INTO arch `dflash` (tensors `markov_w1`/`conf_proj`) → `unknown model architecture: 'dspark'`. #25173's converter is Qwen-only (no Gemma), so no clean path.
  - **qwen3-vl-8b (undrafted today → net-new target):** converted the DeepSpec `dspark_qwen3_8b_block7` checkpoint to upstream format with `convert_hf_to_gguf.py --target-model-dir <Qwen3-8B tok>` (arch dflash + markov, 2.25G bf16). It LOADS cleanly (`draft-dspark` added, block_size=7, n_extract=5, vocab 151936 matches) but **CRASHES on first decode: `llama-graph.cpp:1247 GGML_ASSERT(t_layer_inp[il] != nullptr)`**. ROOT CAUSE: DFlash/DSpark/Eagle3 are target-feature-conditioned — they inject the TARGET's internal hidden-states (n_extract layers); the `qwen3vl` (vision) graph does NOT expose those layer-input tensors → null → abort. This is a target-ARCH incompatibility, fundamental, not a flag. (Contrast: `draft-mtp` with a standalone `--model-draft` assistant does NOT hook target internals → works on VLMs, which is why the gemma QAT-MTP assistant runs fine.)
- **→ DSpark's only viable home on THIS box is the TEXT models (arch `qwen35`): dense 27B / MoE 35B — NOT the VLMs.** But there the native MTP head already captures most of the win (fable-tc dense +121%/+24% measured; MoE +53-73% @0.99), so DSpark is an UPGRADE fight with thin headroom (dense reasoning accept 0.422). Community `Qwen3.6-27B-DFlash/DSpark` GGUFs exist (Alittlehammmer/satgeze) → a dense-27B text A/B is the one remaining non-blocked, non-redundant-ish probe (does DSpark beat the 0.422 reasoning MTP?). MoE 35B-A3B has no community draft. Evidence: `runs/a1-mtp-depth/a5_fable.csv`, reusable A/B drivers `ops/a5_dspark_ab.py` (gemma) + `ops/a5_dspark_qwen.py` (qwen). Probe binary is `dspark-probe`; deploy `lifecycle` untouched (fold #25173 + re-bless only IF a text-model win materializes — the cherry-pick is a banked capability either way).
