#!/usr/bin/env python3
"""Physical Q2_K GGUF and live serving audit for SLX-10."""
...
TASK_ID="BACKLOG-SLX10-PACKED-RUNTIME-02";BASE="http://127.0.0.1:8080";F16=ROOT/"runs/research/BACKLOG-ADAPT06-SLOP-LIVE-05/raw/qwen3.5-0.8b-base-f16.gguf"
...
EXPECTED.pop(ROOT/"config/research_backlog_admissions/BACKLOG-SLX10-PACKED-RUNTIME-01.json")
EXPECTED.pop(ROOT/"runs/research/BACKLOG-SLX10-PACKED-RUNTIME-01/PRE_REGISTRATION.md")
EXPECTED[ROOT/"config/research_backlog_admissions/BACKLOG-SLX10-PACKED-RUNTIME-02.json"]="c73efab4bdd44d8f140d077c16e6e7442c3e21124e792af721336a3eafb07718"
EXPECTED[ROOT/"runs/research/BACKLOG-SLX10-PACKED-RUNTIME-02/PRE_REGISTRATION.md"]="058efe1958e865a35de8e708846cee8cdfa8acc24c24d162dfb930c67fe7bf8c"
...
def execute(outdir):
 ...
 original=service();original_exec=original["values"].get("ExecStart","");binary=original_exec.split("path=",1)[1].split(" ;",1)[0];quantizer=str(pathlib.PurePosixPath(binary).with_name("llama-quantize"));binary_hash=run(["wsl","-d","Ubuntu-24.04","-e","sha256sum",binary])["stdout"].split()[0];quantizer_hash=run(["wsl","-d","Ubuntu-24.04","-e","sha256sum",quantizer])["stdout"].split()[0];q2k=raw/"qwen3.5-0.8b-base-q2_k.gguf";q=run(["wsl","-d","Ubuntu-24.04","-e",quantizer,wsl_path(F16),wsl_path(q2k),"Q2_K"])
 if q["returncode"] or not q2k.is_file():write(raw/"quantization_abort.json",q);raise RuntimeError("quantization failed")
 ...
 try:
  ...
  for arm,model in (("f16",F16),("q2k",q2k)):
...
def main():
 p=argparse.ArgumentParser();p.add_argument("--outdir",type=pathlib.Path,default=ROOT/"runs/research"/TASK_ID);a=p.parse_args();r,m=execute(a.outdir.resolve());passed=all(x["pass"] for x in r["gates"].values());claim="SLX10_Q2K_PHYSICAL_QUALIFIED_R2" if passed else "SLX10_FALSE_POSITIVE_CONFIRMED_R2";failed=[g for g,x in r["gates"].items() if not x["pass"]];(a.outdir/"RESULT.md").write_text(f"# {TASK_ID} result\n\n`{claim}` pending independent AGY review.\n\nQ2_K/F16 file ratio {m['q2k_file_ratio']:.4%}; net VRAM reduction {m['vram_reduction']:.4%}; throughput ratio {m['throughput_ratio']:.4f}; F16/Q2_K accuracy {m['f16_accuracy']:.4%}/{m['q2k_accuracy']:.4%}; exact output rate {m['exact_output_rate']:.4%}. Failed gates: {', '.join(failed) if failed else 'none'}. Scope is the frozen 0.8B model only; IQ2 remains blocked on imatrix. Original service restored.\n",encoding="utf-8");print(json.dumps({"claim":claim,"metrics":m,"gates":r["gates"]},indent=2));return 0
