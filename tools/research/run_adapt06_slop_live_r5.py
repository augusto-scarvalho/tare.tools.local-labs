...
TASK_ID="BACKLOG-ADAPT06-SLOP-LIVE-05";BASE="http://127.0.0.1:8080"
EXPECTED={
ROOT/"config/research_backlog_admissions/BACKLOG-ADAPT06-SLOP-LIVE-05.json":"a162a9f5d798e82646c6537ec4700e8fda547584530888388591a6b48251e9a2",ROOT/"runs/research/BACKLOG-ADAPT06-SLOP-LIVE-05/PRE_REGISTRATION.md":"ae9cb1dcb351de576aea169c3fbeabdc8f4f71c09563ff6e5c4257f4d10eb70b",
...
ROOT/"runs/research/BACKLOG-ADAPT-TRAIN-01/raw/checkpoint_seed_20260824/adapter_config.json":"4872214e344ce266b0b10a1f70db33775224441ff1d78dbe0c7fb4dd70aeac84",ROOT/"runs/research/BACKLOG-ADAPT-TRAIN-01/raw/checkpoint_seed_20260824/adapter_model.safetensors":"05b80090d2d1ba751d48a5032cddec82819a79b9bfb5bd8b05306b85d6ef0122",ROOT/"runs/research/BACKLOG-ADAPT-TRACE-DISTILL-03/raw/checkpoints/seed_20260824_full_trace/adapter_config.json":"4872214e344ce266b0b10a1f70db33775224441ff1d78dbe0c7fb4dd70aeac84",ROOT/"runs/research/BACKLOG-ADAPT-TRACE-DISTILL-03/raw/checkpoints/seed_20260824_full_trace/adapter_model.safetensors":"174832aa1bd25cbc5ed7f0ff717ad253ec94e2c23edc82e6f828ceadeed566b7"}
...
def execute(outdir):
 ...
 original=service();original_exec=original["values"].get("ExecStart","");binary=original_exec.split("path=",1)[1].split(" ;",1)[0];original_model=original_exec.split("-m ",1)[1].split(" ",1)[0]
 binary_hash=run(["wsl","-d","Ubuntu-24.04","--","sha256sum",binary])["stdout"].split()[0];converter="/home/augus/src/slop.cpp-main/convert_lora_to_gguf.py";hf_converter="/home/augus/src/slop.cpp-main/convert_hf_to_gguf.py";python="/home/augus/.venvs/adapt00-20260824/bin/python";base="/home/augus/.cache/huggingface/hub/models--Qwen--Qwen3.5-0.8B-Base/snapshots/dc7cdfe2ee4154fa7e30f5b51ca41bfa40174e68";base_hf="/home/augus/models/adapt00/qwen3.5-0.8b-base-dc7cdfe";converter_hash=run(["wsl","-d","Ubuntu-24.04","--","sha256sum",converter])["stdout"].split()[0];hf_converter_hash=run(["wsl","-d","Ubuntu-24.04","--","sha256sum",hf_converter])["stdout"].split()[0]
 frozen_wsl={"config.json":"b90b86f35c8e6925ef74ee04d0e758f0a845c83a42089ad82bbaa948de9b4204","model.safetensors-00001-of-00001.safetensors":"c2b1e5a17d9c1e27685d92ed9b382911ebb99955ecd89052d1721241adfbab6c","model.safetensors.index.json":"ce9a885efdf27d3664fdef5d512ad365216f1074051ef840c7cd8e5431495d0a"}
 for name,expected in frozen_wsl.items():
  actual=run(["wsl","-d","Ubuntu-24.04","--","sha256sum",f"{base_hf}/{name}"])["stdout"].split()[0]
  if actual!=expected:raise ValueError(f"WSL base hash mismatch {name}: {actual}")
 model=raw/"qwen3.5-0.8b-base-f16.gguf";model_conversion=run(["wsl","-d","Ubuntu-24.04","--",python,hf_converter,base_hf,"--outfile",wsl_path(model),"--outtype","f16"],timeout=1200)
 if model_conversion["returncode"] or not model.is_file():write(raw/"model_conversion_abort.json",model_conversion);raise RuntimeError("base conversion failed")
 adapters=raw/"adapters";adapters.mkdir();sources=[ROOT/"runs/research/BACKLOG-ADAPT-TRAIN-01/raw/checkpoint_seed_20260824",ROOT/"runs/research/BACKLOG-ADAPT-TRACE-DISTILL-03/raw/checkpoints/seed_20260824_full_trace"];outputs=[adapters/"behavioral-f16.gguf",adapters/"trace-f16.gguf"];conversions=[]
 ...
 try:
  ...
  if stopped["returncode"]:raise RuntimeError(f"root systemd stop failed: {stopped}")
  end=time.time()+30
  while time.time()<end and health(8080) is not None:time.sleep(.25)
  if health(8080) is not None:raise RuntimeError("original inference endpoint remained reachable after verified root stop")
  if health(8081)!=200:raise RuntimeError("embedding failed during maintenance")
  args=["wsl","-d","Ubuntu-24.04","--",binary,"-m",wsl_path(model),"--alias","qwen3.5-0.8b-base","--host","0.0.0.0","--port","8080","-ngl","99","-fa","on","--ctx-size","8192","--parallel","4","--jinja","--metrics","--lora",f"{wsl_path(outputs[0])},{wsl_path(outputs[1])}","--lora-init-without-apply"]
  ...
  started_service=run(["wsl","-d","Ubuntu-24.04","-u","root","--","systemctl","start","llm-inference.service"],timeout=60)
  if started_service["returncode"]:raise RuntimeError(f"root systemd start failed: {started_service}")
  try:wait_health(8080,200,240)
  finally:restored=service()
 if stable_exec(restored["values"].get("ExecStart",""))!=stable_exec(original_exec):raise RuntimeError("original executable path/argv not restored")
